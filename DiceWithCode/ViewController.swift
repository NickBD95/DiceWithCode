...
//  DiceWithCode
//
//  Created by Nikolai Grachev on 04.02.2024.
//
...
private class ViewController: UIViewController {
    ...
    private let dices = ["DiceOne", "DiceTwo", "DiceThree", "DiceFour", "DiceFive", "DiceSix"]
    ...
    lazy var rollButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(rollButtonPresed), for: .touchUpInside)
        button.translatesAutoresizingMaskIntoConstraints = false
        return button
    }()
    
    @objc
    func rollButtonPresed() {
        diceOneImageView.image = UIImage(named: dices.randomElement() ?? "DiceOne")
        diceTwoImageView.image = UIImage(named: dices.randomElement() ?? "DiceOne")
    }
    ...
    override func viewDidLoad() {
        ...
        diceTwoImageView.heightAnchor.constraint(equalTo: diceTwoImageView.widthAnchor, multiplier: 1).isActive = true
        ...
        rollButton.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -40).isActive = true
        rollButton.heightAnchor.constraint(equalToConstant: 65).isActive = true
        rollButton.widthAnchor.constraint(equalToConstant: 130).isActive = true
    }
